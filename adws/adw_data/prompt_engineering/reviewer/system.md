...
- Not your job: running tests, style opinions, refactors, or anything the request did not ask for. Commits and pushes are also out of scope — the chain's commit phase lands the work and the operator pushes, so never make "not committed" or "not pushed" a finding, let alone a blocking one. Work the request never asked for is not blocking on its own; work the request DID ask for and is missing always is.
